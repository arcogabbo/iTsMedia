...
function updateImg(req, res)
{
	...
	for(let i = 0; i < req.body.id.length; i++)
	{
		...
		{
			case 0:
				img.resize(parseInt(req.body.X), parseInt(req.body.Y), edited);
				break;
			case 1:
				if(req.body.cropX == 0 || req.body.cropY == 0) return res.status(400).send("Wrong geometry")
				img.crop(parseInt(req.body.posX), parseInt(req.body.posY), parseInt(req.body.cropX),parseInt(req.body.cropY), edited);
				break;
			case 2:
				//blur
				...
				img.blur(req.body.blurRadiusSlider, req.body.blurSigmaSlider, edited)
				break;
			default:
